...
public class CCRecipes {
	...
	static {
		...
		//工作台
		{
			Pair<String, Supplier<CriterionTriggerInstance>> has_any_marshmallow_log = criterion("has_any_marshmallow_log", () -> Utils.hasItemTags(CCItemTags.MARSHMALLOW_LOG)),
		...
		{
			Collections.addAll(CCDatagenManager.recipes,
					licoriceFurnace(() -> licorice.get().getDefaultInstance(), () -> Ingredient.of(licorice_ore.getBlock()))
			);
		}
		//制糖机
		{
			Collections.addAll(CCDatagenManager.recipes,
					sugarFactory(() -> honeycomb.get().getDefaultInstance(), () -> Ingredient.of(honeycomb_block.getBlock()))
			);
		}
	}
	...
	/**
	 * Jump {@link CCRecipes#caramelPortal(Supplier, Supplier, CaramelPortalRecipeBuilder.InDimension)}
	 */
	...
	//Custom//
	private static Consumer<Consumer<FinishedRecipe>> licoriceFurnace(Supplier<ItemStack> output, Supplier<Ingredient> input) {
		return (c) -> {
			ItemStack result = output.get();
			new LicoriceFurnaceRecipeBuilder(result, input.get()).save(c, getID(result.getItem()));
		};
	}
	...
	private static Consumer<Consumer<FinishedRecipe>> sugarFactory(Supplier<ItemStack> output, Supplier<Ingredient> input) {
		return sugarFactory(output, input, false);
	}
	
	private static Consumer<Consumer<FinishedRecipe>> sugarFactory(Supplier<ItemStack> output, Supplier<Ingredient> input, boolean advanced) {
		return (c) -> {
			ItemStack result = output.get();
			new SugarFactoryRecipeBuilder(input.get(), result, advanced).save(c, getID(result.getItem()));
		};
	}
	
	private static Consumer<Consumer<FinishedRecipe>> advancedSugarFactory(Supplier<ItemStack> output, Supplier<Ingredient> input) {
		return sugarFactory(output, input, true);
	}
